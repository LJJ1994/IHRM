...
public class AttendanceController extends BaseController {
    ...
    public Result list(@RequestBody AttePageVO attePageVO) throws ParseException {
        attePageVO.setCompanyId(companyId);
        Map map = atteService.findList(attePageVO);
     ...
     * 归档历史列表
     */
    @RequestMapping(value = "/reports/year" , method = RequestMethod.POST)
    public Result findReportsYear(@RequestBody ArchiveVO archiveVO) {
        archiveVO.setCompanyId(companyId);
        List<ArchiveMonthly> list = archiveService.findByYear(archiveVO.getCompanyId(),
                archiveVO.getDepartmentId(), archiveVO.getYear());
        return new Result<>(ResultCode.SUCCESS , list);
    }
